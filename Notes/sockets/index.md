...
Socket programming is a key skill needed for the final project.
...
### Host Byte Order to Network Byte Order conversion  {#byte-order}
...
See [diagram](https://en.wikipedia.org/wiki/Endianness#Little-endian).

See Wikipedia about *[Endianness](https://en.wikipedia.org/wiki/Endianness)*.
...
The `backlog` argument provides an hint to the system of the number of outstanding connect requests that it should enqueue in behalf of the process.
...
## TCP client/server example {#tcp-example}

In this example, [client-server]({{site.examples}}/client-server), we see a basic TCP client that can talk to our basic TCP server.
...
7* the client-side flow: `socket`, `connect`, `write`, `close`
...
<!-- See the in-class script from [flume](script-flume.txt) and [Mac](script-mac.txt). -->


## HTTP-over-TCP client/server example {#http-example}

In this example, [client-server-http]({{site.examples}}/client-server-http), we enhance the basic example to support HTTP.
...
<!-- See again the in-class script from [flume](script-flume.txt) and [Mac](script-mac.txt). -->
...
In today's [activity](activity.html) your group will copy and install the example code and try it out!
